...
Write-Host "=== Wise ERP - Comercial + Fiscal V3 ===" -ForegroundColor Cyan
...
try {
  ...
  Write-Host "Não foi possível iniciar o PostgreSQL via Docker Compose. Verifique se o banco já está ativo." -ForegroundColor Yellow
}
Set-Location $backend
...
Write-Host "`n[3/6] Criando/aplicando migration Comercial + Fiscal..." -ForegroundColor Cyan
npx prisma migrate dev --name batch_commercial_fiscal_v3 --skip-generate
...
Write-Host "`nLOTE COMERCIAL + FISCAL V3 CONCLUIDO COM SUCESSO." -ForegroundColor Green
